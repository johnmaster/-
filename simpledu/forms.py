...
class RegisterForm(FlaskForm):
    ...
    def validate_username(self, field):
        if not field.data.isalnum():
            raise ValidationError('用户名只能由字母和数字构成')
...
class CourseForm(FlaskForm):
    ...
    description = TextAreaField('CourseIntroduction',validators=[DataRequired(), Length(20, 256)])
    image_url = StringField('CoverImage',validators=[DataRequired(), URL()])
    author_id = IntegerField('AuthorID',validators=[DataRequired(),NumberRange(min=1,message='Invalid User ID')])
...
class LiveForm(FlaskForm):
    name = StringField('CourseName',validators=[DataRequired(),Length(1,128)])
    user_id = IntegerField('Live User ID',validators=[DataRequired(),NumberRange(min=1,message='invalid User ID')])
    submit = SubmitField('Submit')
